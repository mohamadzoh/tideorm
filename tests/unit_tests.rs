...
mod fulltext_tests {
    ...
    fn test_fulltext_config_with_weights() {
        ...
        let config = FullTextConfig::new().language("english").weights(weights);
    ...
    fn test_fulltext_index_postgres_single_column() {
        let index = FullTextIndex::new("idx_articles_title", "articles", vec!["title".to_string()])
            .language("english")
    ...
    fn test_fulltext_index_postgres_gist() {
        let index = FullTextIndex::new("idx_search", "documents", vec!["body".to_string()])
    ...
    fn test_search_result_with_highlights() {
        ...
        let result = SearchResult::new("record".to_string(), 0.8).with_highlights(highlights);
...
mod error_tests {
    ...
    fn test_error_with_context() {
        ...
        assert_eq!(
            err_with_ctx.context().unwrap().table.as_ref().unwrap(),
            "posts"
        );
    }
...
mod schema_tests {
    ...
    fn test_rust_type_to_sql_postgres() {
        ...
        assert_eq!(
            rust_type_to_sql("DateTime", DatabaseType::Postgres),
            "TIMESTAMP"
        );
        assert_eq!(
            rust_type_to_sql("NaiveDateTime", DatabaseType::Postgres),
            "TIMESTAMP"
        );
    ...
    fn test_rust_type_to_sql_mysql() {
        ...
        assert_eq!(
            rust_type_to_sql("DateTime", DatabaseType::MySQL),
            "DATETIME"
        );
    ...
    fn test_rust_type_to_sql_sqlite() {
        ...
        assert_eq!(
            rust_type_to_sql("serde_json::Value", DatabaseType::SQLite),
            "TEXT"
        );
    }
    ...
    fn test_rust_type_to_sql_optional() {
        ...
        assert_eq!(
            rust_type_to_sql("Option<String>", DatabaseType::Postgres),
            "TEXT"
        );
        assert_eq!(
    ...
    fn test_rust_type_to_sql_with_whitespace() {
        ...
        assert_eq!(
            rust_type_to_sql("Option < i64 >", DatabaseType::SQLite),
            "INTEGER"
        );
        assert_eq!(
            rust_type_to_sql("Option < i64 >", DatabaseType::MySQL),
            "BIGINT"
        );
    }
...
mod operator_coverage_tests {
    ...
    fn test_all_pattern_operators() {
        let ops = [Operator::Like, Operator::NotLike];
        assert_eq!(ops.len(), 2);
    }

    #[test]
    fn test_all_membership_operators() {
        let ops = [Operator::In, Operator::NotIn];
        assert_eq!(ops.len(), 2);
    }

    #[test]
    fn test_all_null_operators() {
        let ops = [Operator::IsNull, Operator::IsNotNull];
        assert_eq!(ops.len(), 2);
    }
...
mod attachments_tests {
    ...
    fn test_file_attachment_add_metadata() {
        ...
        assert_eq!(
            attachment.metadata.get("width"),
            Some(&serde_json::json!(1920))
        );
        assert_eq!(
            attachment.metadata.get("height"),
            Some(&serde_json::json!(1080))
        );
        assert_eq!(
            attachment.metadata.get("format"),
            Some(&serde_json::json!("jpeg"))
        );
    }
...
mod translations_tests {
    ...
    fn test_translations_data_with_complex_values() {
        ...
        assert_eq!(
            data.get("tags", "en"),
            Some(&serde_json::json!(["tag1", "tag2"]))
        );
    }
    ...
    fn test_translations_roundtrip() {
        ...
        assert_eq!(
            restored.get("description", "en"),
            original.get("description", "en")
        );
        assert_eq!(
            restored.get("description", "fr"),
            original.get("description", "fr")
        );
    }
}
...
mod cache_tests {
    ...
    use tideorm::cache::{
        CacheConfig, CacheKeyBuilder, CacheOptions, CacheStats, CacheStrategy,
        PreparedStatementCache, PreparedStatementStats, QueryCache,
    };
    ...
    fn test_query_cache_basic_operations() {
        ...
        // Set a value
        cache
            .set("test_key", &"test_value", None, "test_model")
            .unwrap();
...
mod seeding_tests {
    ...
    fn test_seed_status_not_executed() {
        ...
        assert!(display.contains("priority: 50"));
    }
