...
mod unit_tests {
    use super::*;
    use tideorm::tokenization::{TokenConfig, default_encode, default_decode};
    ...
    fn test_token_length() {
        ...
        for id in [0, 1, 100, i64::MAX, i64::MIN] {
            let token = default_encode(id, "User").unwrap();
            assert_eq!(token.len(), 43, "Token should be 43 characters, got {}", token.len());
        }
    }
...
mod tokenizable_trait_tests {
    ...
    fn test_tokenizable_invalid_token_error() {
        ...
        assert!(err.to_string().contains("token"), "Error message should mention 'token'");
    }
    ...
    fn test_multiple_users_unique_tokens() {
        ...
        let tokens: Vec<String> = users
            .iter()
            .map(|u| u.tokenize().unwrap())
            .collect();
        
        // All tokens should be unique
        for i in 0..tokens.len() {
            for j in (i + 1)..tokens.len() {
                assert_ne!(tokens[i], tokens[j], "Tokens for users {} and {} should be different", i + 1, j + 1);
            }
        }
...
mod security_tests {
    ...
    fn test_tokens_not_predictable() {
        ...
        assert!(common_prefix < token1.len(), "Tokens share too much common prefix");
        assert!(common_prefix2 < token2.len(), "Tokens share too much common prefix");
    }
    ...
    fn test_model_name_binding() {
        ...
        assert!(common < 10, "Model-specific tokens share too much in common");
    }
}

mod edge_cases {
    use super::*;
    use tideorm::tokenization::{default_encode, default_decode};
    ...
    fn test_special_char_model_name() {
        ...
        for name in ["User<T>", "My::Nested::Model", "Model-With-Dashes", "model_with_underscores"] {
