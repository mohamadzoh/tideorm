...
use tideorm::query::{Order, ConditionValue, Operator, OrGroup, LogicalOp};
...
mod edge_case_tests {
    ...
    fn test_empty_nested_or_group() {
        let group = OrGroup::new()
            .nested_or(|inner| inner); // Empty nested group
...
mod or_branch_unit_tests {
    use tideorm::query::{OrBranch, ConditionValue, Operator};
...
mod fluent_or_integration_tests {
    ...
    async fn test_all_fluent_or_scenarios() {
        ...
        let test_users = vec![
            // Active admins
            ("Alice Admin", "alice@example.com", "admin", "Engineering", 30, true, true),
            ("Bob Admin", "bob@example.com", "admin", "Marketing", 35, true, false),
            ("Carl Admin Inactive", "carl@example.com", "admin", "Sales", 28, false, true),
            
            // Active moderators
            ("Diana Mod", "diana@example.com", "moderator", "Support", 25, true, true),
            ("Eve Mod Inactive", "eve@example.com", "moderator", "HR", 40, false, true),
            ("Frank Mod", "frank@example.com", "moderator", "Engineering", 32, true, false),
            
            // Editors
            ("Grace Editor", "grace@example.com", "editor", "Marketing", 27, true, true),
            ("Henry Editor Inactive", "henry@example.com", "editor", "Sales", 45, false, false),
            ("Ivy Editor", "ivy@example.com", "editor", "Engineering", 29, true, true),
            
            // Regular users
            ("Jack User", "jack@example.com", "user", "Support", 22, true, false),
            ("Kate User Inactive", "kate@example.com", "user", "HR", 38, false, true),
            ("Leo User", "leo@example.com", "user", "Engineering", 31, true, true),
            
            // Guests
            ("Mike Guest", "mike@example.com", "guest", "Marketing", 24, true, false),
            ("Nancy Guest Inactive", "nancy@example.com", "guest", "Sales", 50, false, false),
        ];
        ...
        assert_eq!(results.len(), 9, "Expected 9 users (3 admin + 3 moderator + 3 editor)");
        
        for user in &results {
            assert!(
                user.role == "admin" || user.role == "moderator" || user.role == "editor",
                "Unexpected role: {}", user.role
            );
        }
        println!(" PASSED\n");
        ...
        let query = OrTestUser::query()
            ...
            .begin_or()
                ...
                .or_where_eq("role", "moderator").and_where_eq("active", false)
                .or_where_eq("role", "editor")
            .end_or();
        ...
        for user in &results {
            println!("  - {} (role: {}, active: {})", user.name, user.role, user.active);
        }
        ...
        for user in &results {
            ...
            assert!(
                ...
                "User {} has unexpected role {}", user.name, user.role
            );
        }
        println!(" PASSED\n");
        ...
        let query = OrTestUser::query()
            ...
            .begin_or()
                ...
                .or_where_eq("role", "moderator").and_where_eq("department", "Engineering")
                .or_where_eq("role", "editor")
            .end_or();
        ...
        for user in &results {
            ...
            let matches_criteria = 
                ...
                user.role == "editor";
            assert!(matches_criteria, "User {} doesn't match criteria", user.name);
        }
        println!(" PASSED\n");
        ...
        let query = OrTestUser::query()
            .begin_or()
                .or_where_eq("role", "admin").and_where_lt("age", 30)
                .or_where_eq("role", "moderator").and_where_gt("age", 35)
                .or_where_eq("role", "editor").and_where_eq("verified", true)
            .end_or();
        ...
        for user in &results {
            println!("  - {} (role: {}, active: {}, verified: {}, age: {}, dept: {})", 
        ...
        for user in &results {
            ...
            assert!(branch1 || branch2 || branch3, "User {} doesn't match any branch", user.name);
        }
        println!(" PASSED\n");
        ...
        for user in &results {
            println!("  - {} (role: {}, dept: {})", user.name, user.role, user.department);
        }
        
        for user in &results {
            let matches = 
                (user.role == "admin" && (user.department == "Engineering" || user.department == "Marketing")) ||
        ...
        let query = OrTestUser::query()
            .begin_or()
                .or_where_eq("role", "admin").and_where_between("age", 25, 35)
                .or_where_eq("role", "moderator").and_where_between("age", 30, 45)
            .end_or();
        ...
        for user in &results {
            let matches = 
                (user.role == "admin" && user.age >= 25 && user.age <= 35) ||
                (user.role == "moderator" && user.age >= 30 && user.age <= 45);
            assert!(matches, "User {} (age: {}) doesn't match BETWEEN criteria", user.name, user.age);
        }
        println!(" PASSED\n");
        ...
        assert_eq!(count as usize, results.len(), "Count should match actual results");
        println!(" PASSED\n");
        ...
        for i in 1..results.len() {
            assert!(results[i-1].age >= results[i].age, 
        ...
        assert!(sql_lower.contains("active"), "SQL should contain active column");
        ...
        let query = OrTestUser::query()
            .begin_or()
                .or_where_eq("role", "admin").and_where_like("name", "A%")
        ...
        for user in &results {
            let matches = 
                (user.role == "admin" && user.name.starts_with("A")) ||
