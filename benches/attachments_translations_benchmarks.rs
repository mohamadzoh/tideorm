...
use tideorm::translations::{TranslationsData, TranslationInput, HasTranslations, TranslationError};
...
impl HasTranslations for BenchProduct {
    ...
    fn allowed_languages() -> Vec<String> {
        vec!["en".to_string(), "ar".to_string(), "fr".to_string(), "es".to_string(), "de".to_string()]
    }
...
fn bench_file_attachment_creation(c: &mut Criterion) {
    ...
    group.bench_function("create_simple", |b| {
        b.iter(|| {
            black_box(FileAttachment::new("uploads/2024/01/image.jpg"))
        });
    });
...
fn bench_files_data_operations(c: &mut Criterion) {
    ...
    group.bench_function("get_one", |b| {
        ...
        files.set_one("thumbnail", FileAttachment::new("thumb.jpg"));
        b.iter(|| {
            black_box(files.get_one("thumbnail"))
        });
    });
    ...
    group.bench_function("add_many_single", |b| {
        ...
        b.iter(|| {
            counter += 1;
            files.add_many("images", FileAttachment::new(&format!("img{}.jpg", counter)));
            black_box(&files);
        });
    });
    ...
    group.bench_function("get_many_10_items", |b| {
        ...
        b.iter(|| {
            black_box(files.get_many("images"))
        });
    });
...
fn bench_attachment_trait_operations(c: &mut Criterion) {
    ...
    group.bench_function("attach_single", |b| {
        ...
        b.iter(|| {
            counter += 1;
            product.attach("images", &format!("uploads/img{}.jpg", counter)).unwrap();
            black_box(&product);
        });
    });
...
fn bench_translation_trait_operations(c: &mut Criterion) {
    ...
    group.bench_function("get_all_translations", |b| {
        ...
        b.iter(|| {
            black_box(product.get_all_translations("name").unwrap())
        });
    });
    ...
    group.bench_function("to_translated_json", |b| {
        let mut product = BenchProduct::new(1);
        product.set_translation("name", "ar", "منتج").unwrap();
        product.set_translation("description", "ar", "وصف المنتج").unwrap();
        product.set_translation("name", "en", "Product").unwrap();
        product.set_translation("description", "en", "Product description").unwrap();
        
        let mut opts = HashMap::new();
        opts.insert("language".to_string(), "ar".to_string());
        
        b.iter(|| {
            black_box(product.to_translated_json(Some(opts.clone())))
        });
    });
...
fn bench_translations_serialization(c: &mut Criterion) {
    ...
    group.bench_function("to_json_small", |b| {
        ...
        b.iter(|| {
            black_box(data.to_json())
        });
    });
    
    group.bench_function("to_json_large", |b| {
        let mut data = TranslationsData::new();
        for field in &["name", "description", "short_desc", "meta_title", "meta_desc"] {
    ...
    group.bench_function("from_json", |b| {
        ...
        b.iter(|| {
            black_box(TranslationsData::from_json(&json))
        });
    });
...
fn bench_combined_operations(c: &mut Criterion) {
    ...
    group.bench_function("full_product_setup", |b| {
        b.iter(|| {
            ...
            // Set translations
            product.set_translation("name", "en", "Wireless Headphones").unwrap();
            product.set_translation("name", "ar", "سماعات لاسلكية").unwrap();
            product.set_translation("description", "en", "High-quality wireless headphones").unwrap();
            product.set_translation("description", "ar", "سماعات لاسلكية عالية الجودة").unwrap();
            
            // Attach files
            product.attach("thumbnail", "uploads/thumb.jpg").unwrap();
            product.attach_many("images", vec!["img1.jpg", "img2.jpg", "img3.jpg"]).unwrap();
            product.attach_many("documents", vec!["manual.pdf", "warranty.pdf"]).unwrap();
    ...
    for size in [10, 50, 100].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.iter(|| {
                    let products: Vec<BenchProduct> = (0..size).map(|i| {
                        let mut product = BenchProduct::new(i as i64);
                        product.set_translation("name", "en", format!("Product {}", i)).unwrap();
                        product.set_translation("name", "ar", format!("منتج {}", i)).unwrap();
                        product.attach("thumbnail", &format!("thumb{}.jpg", i)).unwrap();
                        product
                    }).collect();
                    black_box(products)
                });
