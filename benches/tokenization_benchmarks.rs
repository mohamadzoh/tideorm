...
// ============================================================================
// TOKEN CONFIGURATION (mirrors TideORM's TokenConfig)
// ============================================================================
...
impl Default for TokenConfig {
    fn default() -> Self {
        Self {
            encryption_key: "default-key-for-development-only!".to_string(),
            encoder: None,
            decoder: None,
        }
    }
...
fn base64_url_decode(input: &str) -> Option<Vec<u8>> {
    ...
    while i < bytes.len() {
        ...
        let b1 = if i + 1 < bytes.len() { char_to_value(bytes[i + 1])? } else { 0 };
        let b2 = if i + 2 < bytes.len() { char_to_value(bytes[i + 2])? } else { 0 };
        let b3 = if i + 3 < bytes.len() { char_to_value(bytes[i + 3])? } else { 0 };
...
mod hex {
    pub fn encode(data: impl AsRef<str>) -> String {
        data.as_ref().as_bytes().iter().map(|b| format!("{:02x}", b)).collect()
    }
...
fn bench_token_decoding(c: &mut Criterion) {
    ...
    let tokens: Vec<_> = ids.iter().map(|&id| default_encode(id, "BenchmarkModel", &key)).collect();
    ...
    let prefix_tokens: Vec<_> = ids.iter().map(|&id| simple_prefix_encode(id, "BenchmarkModel")).collect();
    ...
    let hex_tokens: Vec<_> = ids.iter().map(|&id| hex_encode(id, "BenchmarkModel")).collect();
...
fn bench_model_specificity(c: &mut Criterion) {
    ...
    let models = ["User", "Product", "Order", "Invoice", "VeryLongModelNameForTesting"];
    ...
    for (model, token) in models.iter().zip(tokens.iter()) {
        group.bench_with_input(BenchmarkId::new("decode", model), &(model, token), |b, (model, token)| {
...
fn bench_tamper_detection(c: &mut Criterion) {
    ...
    if tampered_bytes.len() > 5 {
        tampered_bytes[5] = if tampered_bytes[5] == b'a' { b'b' } else { b'a' };
    }
    ...
    group.bench_function("tampered_token", |b| {
        b.iter(|| black_box(default_decode(black_box(&tampered_char), "TamperTest", &key)))
    });
...
fn bench_batch_operations(c: &mut Criterion) {
    ...
    let tokens: Vec<_> = ids.iter().map(|&id| default_encode(id, "BatchModel", &key)).collect();
...
fn bench_key_lengths(c: &mut Criterion) {
    ...
    let keys = [
        ...
        ("64_chars", "1234567890123456789012345678901234567890123456789012345678901234"),
        ("128_chars", "12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678"),
    ];
    ...
    for (name, key_str) in keys {
        ...
        group.bench_with_input(BenchmarkId::new("decode", name), &(key, token), |b, (key, token)| {
...
fn bench_iv_generation(c: &mut Criterion) {
    c.bench_function("iv_generation", |b| {
        b.iter(|| black_box(generate_iv()))
    });
}

fn bench_token_config(c: &mut Criterion) {
    let mut group = c.benchmark_group("token_config");
    
    group.bench_function("config_creation_default", |b| {
        b.iter(|| black_box(TokenConfig::default()))
    });
    
    group.bench_function("config_creation_custom", |b| {
        b.iter(|| black_box(TokenConfig::new("custom-encryption-key-for-bench")))
    });
    
    group.bench_function("config_clone", |b| {
        let config = TokenConfig::new("custom-encryption-key-for-bench");
