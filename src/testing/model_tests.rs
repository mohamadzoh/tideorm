...
fn test_with_relations_preserves_deserialized_cached_relations() {
    let mut model: TranslationRelationUser = serde_json::from_value(serde_json::json!({
        "id": 1,
        "title": "Default Title",
        "translations": null,
        "profile": {
    ...
    model = model.with_relations();

    assert_eq!(model.profile.foreign_key, "user_id");
    assert_eq!(model.profile.local_key, "id");
    assert_eq!(model.profile.get_cached().map(|profile| profile.id), Some(10));
...
fn test_model_relation_serialization_round_trip_preserves_cached_relations() {
    ...
    let mut round_trip: TranslationRelationUser =
        serde_json::from_value(value).expect("model should deserialize after serialization round trip");

    round_trip = round_trip.with_relations();

    assert_eq!(round_trip.profile.get_cached().map(|profile| profile.id), Some(10));
    assert_eq!(round_trip.posts.get_cached().map(|posts| posts.len()), Some(1));
    assert_eq!(round_trip.posts.get_cached().unwrap()[0].id, 20);
    assert_eq!(round_trip.roles.get_cached().map(|roles| roles.len()), Some(1));
...
fn test_with_relations_preserves_deserialized_belongs_to_cache() {
    let mut model: AttachmentRelationPost = serde_json::from_value(serde_json::json!({
        "id": 3,
        "user_id": 7,
        "files": null,
        "author": {
